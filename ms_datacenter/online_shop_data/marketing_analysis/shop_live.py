...
while i <= 4:
    ...
    # 观看量
    watch_num = random.randint(1,10000)

    # 新增粉丝量
    new_add_funs = random.randint(1,watch_num)

    # 商品点击量
    goods_clicks = random.randint(1,10000)
    ...
    # 成功退款金额
    refund_money = random.uniform(1,transaction_amount)

    # 支付件数
    pay_num = random.randint(0,pay_number)

    # 引导加购数
    guide_buy_back_num = random.randint(0,1000)

    # 引导加购人数
    guide_buy_back_people = random.randint(0,500)

    # 引导支付子订单数据
    guide_pay_sub_order_num = random.randint(0,500)

    # 引导进店人数
    guide_enter_people_num = random.randint(0,1000)

    # 引导进店量
    guide_enter_num = random.randint(0,1000)

    # 引导支付人数
    guide_pay_people_num = random.randint(0, guide_enter_people_num)
    ...
    # 打赏音浪
    reward_soundbyte = random.randint(1,200)

    # 总消耗费用
    consumption_cost_count = random.uniform(0,10000)

    # 打赏金额
    reward_amount = random.randint(0,2000)

    # 直播间订单量（即时影响）
    live_room_order_num_now = random.randint(0,1000)

    # 直播间订单量（长效影响-7天）
    live_room_order_num_week = random.randint(0,1000)
    ...
    print("店铺ID：%s  日期：%s  直播场次：%s  观看量：%d  新增粉丝量：%d  商品点击量：%d  支付笔数：%d  成交金额：%.2f  成功退款金额：%.2f  "
          "支付件数：%d  引导加购数：%d  引导加购人数：%d  引导支付子订单数据：%d  引导进店人数：%d  引导进店量：%d  引导支付人数：%d  定向计划点击量：%d  "
          "定向计划支付笔数：%d  定向计划成交金额：%.2f  淘宝支付笔数：%d  淘宝客付款预估收入：%.2f  打赏音浪：%d  总消耗费用：%.2f  "
          "打赏金额：%.2f  直播间订单量（即时影响）:%d  直播间订单量（长效影响-7天）：%d  直播间成交金额（即时影响）：%d  直播间成交金额（长效影响-7天）：%.2f" %
          (shop_id,date_time,live_num,watch_num,new_add_funs,goods_clicks,pay_number,transaction_amount,refund_money,pay_num,
           guide_buy_back_num,guide_buy_back_people,guide_pay_sub_order_num,guide_enter_people_num,guide_enter_num,guide_pay_people_num,
           orientation_plan_clicks,orientation_plan_pay_number,orientation_plan_transaction_amount,tbk_pay_number,tbk_income_payment,reward_soundbyte,
           consumption_cost_count,reward_amount,live_room_order_num_now,live_room_order_num_week,live_room_deal_amount_now,live_room_deal_amount_week))
...
df = pd.DataFrame(data_list,columns=["店铺ID","日期","直播场次","观看量","新增粉丝量","商品点击量","支付笔数","成交金额","成功退款金额",
          "支付件数","引导加购数","引导加购人数","引导支付子订单数据","引导进店人数","引导进店量","引导支付人数","定向计划点击量","定向计划支付笔数",
        "定向计划成交金额","淘宝支付笔数","淘宝客付款预估收入","打赏音浪","总消耗费用","打赏金额","直播间订单量（即时影响）",
        "直播间订单量（长效影响-7天）","直播间成交金额（即时影响）","直播间成交金额（长效影响-7天）"])
