...
# 设置初始日期
date_time = datetime.date(2021, 1, 30)
...
while z <= 3:
    ...
    order_distribution_resources_list = ["精华", "面霜", "乳液", "洗面奶", "面膜", "隔离霜", "气垫bb", "粉底液", "唇釉", "美妆蛋",
                                         "眼线笔", "定妆喷雾", "小金块", "小银管"]
    ...
    # 99 % 进度播放量
    ninety_nine_play_number = random.randint(0, play_number - twenty_five_play_number - fifty_play_number -
                                             seventy_five_play_number)
    ...
    print(
        ...
        (date_time, plan_ID, video_link, KOL, buy_CPM_number, round(consumption_cost,2), order_distribution_resources,
         exposure, click_number, transformation_number, likes, comments, forwarding_volume, collection_volume,
         landing_page_PV, landing_page_UV, payments_number, pay_number, round(gvm,2), play_number,effective_play_number,
         twenty_five_play_number, fifty_play_number, seventy_five_play_number, ninety_nine_play_number,
         average_play_time, topic_play_times, flow_share_pace, activity_share_number, private_letter_number,
         homepage_visit_number)
    )
    z += 1
3
...
# 将字段和值一一匹配
df = pd.DataFrame(data_list, columns=['日期', '计划ID', '视频链接', '投放达人', '采买cpm数量', '消耗费用', '随单配送资源', '曝光量',
                                      '点击量', '转化数', '点赞量', '评论量', '转发量', '收藏量', '落地页PV', '落地页UV', '支付笔数',
                                      '支付件数', '成交金额', '播放量', '有效播放量', '25%进度播放量', '50%进度播放量', '75%进度播放量',
                                      '99%进度播放量', '平均单次播放时长', '话题播放次数', '流量分成进度', '活动分享数', '私信数', '主页访问量'])
