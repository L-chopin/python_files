...
while i <= 4:
    ...
    print("日期：%s  店铺ID：%s  渠道：%s  今日直播场次：%d  实际合作费用：%.2f  预估佣金费用：%.2f  成交金额：%.2f  点击量：%d  支付笔数：%d  支付件数；%d" %
          (date_time,shop_id,channel,live_today,cooperation_costs,commission_costs,gvm,clicks,payments_number,pay_number))

    i += 1

df = pandas.DataFrame(data_list,columns=['日期','店铺ID','渠道','今日直播场次','实际合作费用','预估佣金费用','成交金额','点击量','支付笔数','支付件数'])
